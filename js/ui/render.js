  ...
  function renderComparison(results) {
    ...
    <table class="compare w-full">
      ...
      <tbody>
        ...
        ${results.some((r) => r.balanceAtHorizon > 0) ? `<tr><td>Remaining balance at month ${state.horizon}</td>${results.map((r) => `<td>${r.balanceAtHorizon ? fmt(r.balanceAtHorizon) : "—"}</td>`).join("")}</tr>` : ""}
      </tbody>
    </table>`
  }
